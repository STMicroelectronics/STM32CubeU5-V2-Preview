...
static hal_tim_handle_t hTIM1;
  static hal_dma_node_t DMA_Node_1;
...
hal_tim_handle_t *mx_tim1_hal_init(void)
{
  ...
  if (HAL_TIM_OC_EnableChannelComparePreload(&hTIM1, HAL_TIM_CHANNEL_1) != HAL_OK)
  {
    return NULL;
  }
  ...
  HAL_GPIO_Init(HAL_GPIOE, HAL_GPIO_PIN_9, &gpio_config);
  ...
  xfer_cfg_tim1_cc1_dma.request         = HAL_GPDMA1_REQUEST_TIM1_CC1;
  ...
  if (HAL_DMA_SetConfigPeriphLinkedListCircularXfer(&hGPDMA1_CH0, &DMA_Node_1, &xfer_cfg_tim1_cc1_dma) != HAL_OK)
  {
    return NULL;
  }
  ...
  /* Link Capture/Compare 1 DMA requests handle to the TIM handle */
  if (HAL_TIM_SetDMA(&hTIM1, HAL_TIM_DMA_ID_CC1, &hGPDMA1_CH0) != HAL_OK)
  {
    return NULL;
  }
...
void mx_tim1_hal_deinit(void)
{
  ...
  HAL_GPIO_DeInit(HAL_GPIOE, HAL_GPIO_PIN_9);
