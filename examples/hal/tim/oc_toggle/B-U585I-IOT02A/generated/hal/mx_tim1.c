...
hal_tim_handle_t *mx_tim1_hal_init(void)
{
  ...
  if (HAL_TIM_OC_EnableChannelComparePreload(&hTIM1, HAL_TIM_CHANNEL_4) != HAL_OK)
  {
    return NULL;
  }
  ...
  /* GPIO configuration for channel4 */
  ...
  gpio_config.speed       = HAL_GPIO_SPEED_FREQ_LOW;
  gpio_config.alternate   = HAL_GPIO_AF_1;
  HAL_GPIO_Init(HAL_GPIOE, HAL_GPIO_PIN_14, &gpio_config);
...
void mx_tim1_hal_deinit(void)
{
  ...
  /* Deinitialization of TIM1: TIM1_CH4  */
  HAL_GPIO_DeInit(HAL_GPIOE, HAL_GPIO_PIN_14);
