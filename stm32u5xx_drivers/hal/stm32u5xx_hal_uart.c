...
/** @brief  Check UART Baud rate.
  ...
  */
#define IS_UART_BAUD_RATE(baud_rate) ((baud_rate) <= 20000000U && ((baud_rate) != 0U))
...
/**
  ...
  */
#define IS_UART_WAKEUP_SELECTION(wakesel) (((wakesel) == HAL_UART_WAKEUP_ON_STARTBIT)              \
...
/**
  ...
  * @param polarity UART Trigger polarity.
  * @retval SET (polarity is valid) or RESET (polarity is invalid)
  */
#define IS_UART_TRIGGER_POLARITY(polarity)  (((polarity) == HAL_UART_AM_TRIG_RISING)      \
                                             || ((polarity) == HAL_UART_AM_TRIG_FALLING))

/**
  * @brief Ensure that UART or LPUART Trigger is valid.
  * @param trigger UART Trigger.
  * @retval SET (trigger is valid) or RESET (trigger is invalid)
  */
#if defined(COMP2)
#define IS_UART_TRIGGER(trigger)  (((trigger) == HAL_UART_AM_TRIG_GPDMA1_CH0_TC)       \
                                   || ((trigger) == HAL_UART_AM_TRIG_GPDMA1_CH1_TC)    \
                                   || ((trigger) == HAL_UART_AM_TRIG_GPDMA1_CH2_TC)    \
                                   || ((trigger) == HAL_UART_AM_TRIG_GPDMA1_CH3_TC)    \
                                   || ((trigger) == HAL_UART_AM_TRIG_EXTI6)            \
                                   || ((trigger) == HAL_UART_AM_TRIG_EXTI9)            \
                                   || ((trigger) == HAL_UART_AM_TRIG_LPTIM1_CH1)       \
                                   || ((trigger) == HAL_UART_AM_TRIG_LPTIM2_CH1)       \
                                   || ((trigger) == HAL_UART_AM_TRIG_COMP1_OUT)        \
                                   || ((trigger) == HAL_UART_AM_TRIG_COMP2_OUT)        \
                                   || ((trigger) == HAL_UART_AM_TRIG_RTC_ALRA_TRG)     \
                                   || ((trigger) == HAL_UART_AM_TRIG_RTC_WUT_TRG)      \
                                   || ((trigger) == HAL_LPUART_AM_TRIG_LPDMA1_CH0_TC)  \
                                   || ((trigger) == HAL_LPUART_AM_TRIG_LPDMA1_CH1_TC)  \
                                   || ((trigger) == HAL_LPUART_AM_TRIG_LPDMA1_CH2_TC)  \
                                   || ((trigger) == HAL_LPUART_AM_TRIG_LPDMA1_CH3_TC)  \
                                   || ((trigger) == HAL_LPUART_AM_TRIG_EXTI6)          \
                                   || ((trigger) == HAL_LPUART_AM_TRIG_EXTI8)          \
                                   || ((trigger) == HAL_LPUART_AM_TRIG_LPTIM1_CH1)     \
                                   || ((trigger) == HAL_LPUART_AM_TRIG_LPTIM3_CH1)     \
                                   || ((trigger) == HAL_LPUART_AM_TRIG_COMP1_OUT)      \
                                   || ((trigger) == HAL_LPUART_AM_TRIG_COMP2_OUT)      \
                                   || ((trigger) == HAL_LPUART_AM_TRIG_RTC_ALRA_TRG)   \
                                   || ((trigger) == HAL_LPUART_AM_TRIG_RTC_WUT_TRG))
...
/** @addtogroup UART_Exported_Functions_Group9
  * @{
...
/**
  ...
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
  * @warning When using this API, TE block will be reset to let the autonomous mode controlling it.
  * @retval HAL_OK UART instance has been correctly configured.
  */
hal_status_t HAL_UART_AM_EnablePacketTrigger(const hal_uart_handle_t *huart)
{
  ...
  ASSERT_DBG_PARAM(huart != NULL);
  p_uartx = UART_GET_INSTANCE(huart);
  ASSERT_DBG_PARAM(IS_UART_AUTONOMOUS_INSTANCE(p_uartx));
  ...
  LL_USART_Enable_SelectedTrigger(p_uartx);
...
/**
  ...
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
  * @warning When using this API, TE block will be set back for the standard mode to be functional.
  * @retval HAL_OK UART instance has been correctly configured.
  */
hal_status_t HAL_UART_AM_DisablePacketTrigger(const hal_uart_handle_t *huart)
{
  ...
  LL_USART_Disable_SelectedTrigger(p_uartx);
...
hal_uart_am_packet_trig_status_t HAL_UART_AM_IsEnabledPacketTrigger(const hal_uart_handle_t *huart)
{
  ...
  return (hal_uart_am_packet_trig_status_t)LL_USART_IsEnabled_SelectedTrigger(p_uartx);
}

/**
  * @brief Set the UART Autonomous Packet Trigger configuration passed in parameters into the handler instance
  *        registers.
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
  * @param p_config Pointer to a hal_uart_autonomous_mode_packet_trigger_config_t.
  * @retval HAL_OK UART instance has been correctly configured.
  * @retval HAL_INVALID_PARAM p_config is NULL.
  */
                                                ...
                                                const hal_uart_am_trig_config_t *p_config)
{
  ...
  ASSERT_DBG_PARAM(p_config != NULL);
  p_uartx = UART_GET_INSTANCE(huart);
  ASSERT_DBG_PARAM(IS_UART_TRIGGER_POLARITY(p_config->trigger_polarity));
  ASSERT_DBG_PARAM(IS_UART_TRIGGER(p_config->source));
  ASSERT_DBG_PARAM(IS_UART_AUTONOMOUS_INSTANCE(p_uartx));
  ...
  if (IS_LPUART_INSTANCE(p_uartx))
  {
    /* need to check if we are using a LPUART Trigger */
    ASSERT_DBG_PARAM((p_config->source & LL_LPUART_TRIG_MASK) == LL_LPUART_TRIG_MASK);

    temp_src = ((uint32_t)p_config->source & ~LL_LPUART_TRIG_MASK);
  }
  else
  {
    /* need to check if we are using a USART Trigger */
    ASSERT_DBG_PARAM((p_config->source & LL_USART_TRIG_MASK) == LL_USART_TRIG_MASK);

    temp_src = ((uint32_t)p_config->source & ~LL_USART_TRIG_MASK);
  }
  LL_USART_AM_ConfigPacketTrigger(p_uartx, (uint32_t)p_config->trigger_polarity, temp_src,
...
/**
  * @brief Get the UART Autonomous Packet Trigger configuration according to the handler instance registers.
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
  * @param p_config Pointer to a hal_uart_autonomous_mode_packet_trigger_config_t.
  */
                                        ...
                                        hal_uart_am_trig_config_t *p_config)
{
  ...
  reg_temp = LL_USART_READ_REG(p_uartx, AUTOCR);

  if (IS_LPUART_INSTANCE(p_uartx))
  {
    p_config->source = (hal_uart_am_trig_source_t)
                       (uint32_t)((reg_temp & USART_AUTOCR_TRIGSEL) | LL_LPUART_TRIG_MASK);
  }
  if (IS_UART_INSTANCE(p_uartx))
  {
    p_config->source = (hal_uart_am_trig_source_t)(uint32_t)
                       ((reg_temp & USART_AUTOCR_TRIGSEL) | LL_USART_TRIG_MASK);
  }
  p_config->trigger_polarity = (hal_uart_am_trig_polarity_t)(uint32_t)(reg_temp & USART_AUTOCR_TRIGPOL);

  p_config->packet_size_byte = (reg_temp & USART_AUTOCR_TDN);
}
...
hal_status_t HAL_UART_AM_EnableIdle(const hal_uart_handle_t *huart)
{
  ...
  LL_USART_Enable_AutonomousSendIdleFrame(p_uartx);
...
hal_status_t HAL_UART_AM_DisableIdle(const hal_uart_handle_t *huart)
{
  ...
  LL_USART_Disable_AutonomousSendIdleFrame(p_uartx);
...
/**
  ...
  * @param huart Pointer to a \ref hal_uart_handle_t structure which contains the UART instance.
  * @retval hal_uart_am_idle_frame_status_t Current Autonomous Mode Idle Frame Sending status.
  */
hal_uart_am_idle_frame_status_t HAL_UART_AM_IsEnabledIdle(const hal_uart_handle_t *huart)
{
  ...
  return (hal_uart_am_idle_frame_status_t)
         LL_USART_IsEnabled_AutonomousSendIdleFrame(p_uartx);
}
/**
  * @}
  */
