...
/** @brief  Check USART Baud rate.
  ...
  */
#define IS_USART_BAUD_RATE(baud_rate) ((baud_rate) <= 20000000U     \
                                       && (baud_rate) > 0U)
...
/**
  ...
  * @param polarity USART Trigger polarity.
  * @retval 1U (polarity is valid) or 0U (polarity is invalid)
  */
#define IS_USART_TRIGGER_POLARITY(polarity)  (((polarity) == HAL_USART_AM_TRIG_RISING) \
                                              || ((polarity) == HAL_USART_AM_TRIG_FALLING))

/**
  * @brief Ensure that USART Trigger is valid.
  * @param trigger USART Trigger.
  * @retval 1U (trigger is valid) or 0U (trigger is invalid)
  */
#if defined (COMP2)

#define IS_USART_TRIGGER(trigger)  (((trigger) == HAL_USART_AM_TRIG_GPDMA1_CH0_TC)    \
                                    || ((trigger) == HAL_USART_AM_TRIG_GPDMA1_CH1_TC) \
                                    || ((trigger) == HAL_USART_AM_TRIG_GPDMA1_CH2_TC) \
                                    || ((trigger) == HAL_USART_AM_TRIG_GPDMA1_CH3_TC) \
                                    || ((trigger) == HAL_USART_AM_TRIG_EXTI6)         \
                                    || ((trigger) == HAL_USART_AM_TRIG_EXTI9)         \
                                    || ((trigger) == HAL_USART_AM_TRIG_LPTIM1_CH1)    \
                                    || ((trigger) == HAL_USART_AM_TRIG_LPTIM2_CH1)    \
                                    || ((trigger) == HAL_USART_AM_TRIG_COMP1_OUT)     \
                                    || ((trigger) == HAL_USART_AM_TRIG_COMP2_OUT)     \
                                    || ((trigger) == HAL_USART_AM_TRIG_RTC_ALRA_TRG)  \
                                    || ((trigger) == HAL_USART_AM_TRIG_RTC_WUT_TRG))
#else
#define IS_USART_TRIGGER(trigger)  (((trigger) == HAL_USART_AM_TRIG_GPDMA1_CH0_TC)    \
                                    || ((trigger) == HAL_USART_AM_TRIG_GPDMA1_CH1_TC) \
                                    || ((trigger) == HAL_USART_AM_TRIG_GPDMA1_CH2_TC) \
...
/** @addtogroup USART_Exported_Functions_Group4
  * @{
  This subsection provides a set of functions allowing to use the autonomous mode feature for the USARTx instance.
  Prior to use the autonomous mode feature, one has to configure one's instance in synchronous
  mode with HAL_USART_SetConfig().
  A set of functions is provided to use the autonomous mode feature:
    - HAL_USART_AM_EnablePacketTrigger(): Enable the autonomous mode feature
    - HAL_USART_AM_DisablePacketTrigger(): Disable the autonomous mode feature
    - HAL_USART_AM_IsEnabledPacketTrigger(): Check if the autonomous mode feature is enabled
    - HAL_USART_AM_SetConfigPacketTrigger(): Set the configuration of the autonomous mode feature
    - HAL_USART_AM_GetConfigPacketTrigger(): Retrieve the configuration of the autonomous mode feature
    - HAL_USART_AM_EnableIdle(): Enable the autonomous mode idle frame sending feature
    - HAL_USART_AM_DisableIdle(): Disable the autonomous mode idle frame sending feature
    - HAL_USART_AM_IsEnabledIdle(): Check if the autonomous mode idle frame sending feature is enabled

  The feature has been designed to be used following the procedure:
    - HAL_USART_AM_SetConfigPacketTrigger()
    - HAL_USART_AM_EnableIdle() (If needed)
    - HAL_USART_AM_EnablePacketTrigger()

  */

/* Autonomous Mode Control functions  **********************************************/
/**
  * @brief Enable the Autonomous Mode Packet Trigger into the handler instance registers.
  * @param husart Pointer to a \ref hal_usart_handle_t structure which contains the USART instance.
  * @warning  When using this API, Transmit Enable (TE) bit in register USART_CR1 will be reset to let the
  *           autonomous mode controlling it.
  * @retval HAL_OK USART instance has been correctly configured.
  */
hal_status_t HAL_USART_AM_EnablePacketTrigger(const hal_usart_handle_t *husart)
{
  USART_TypeDef *p_usartx;
  ...
  ASSERT_DBG_PARAM(IS_UART_AUTONOMOUS_INSTANCE(p_usartx));

  ASSERT_DBG_STATE(husart->global_state, HAL_USART_STATE_IDLE);

  USART_ENSURE_INSTANCE_DISABLED(p_usartx);

  LL_USART_DisableDirectionTx(p_usartx);

  LL_USART_Enable_SelectedTrigger(p_usartx);
...
/**
  * @brief Disable the Autonomous Mode Packet Trigger into the handler instance registers.
  * @param husart Pointer to a \ref hal_usart_handle_t structure which contains the USART instance.
  * @warning When using this API, Transmit Enable (TE) bit in register USART_CR1 be set back for the
  *          standard mode to be functional.
  * @retval HAL_OK USART instance has been correctly configured.
  */
hal_status_t HAL_USART_AM_DisablePacketTrigger(const hal_usart_handle_t *husart)
{
  USART_TypeDef *p_usartx;
  ...
  LL_USART_Disable_SelectedTrigger(p_usartx);

  LL_USART_EnableDirectionTx(p_usartx);
...
/**
  * @brief Return the Autonomous Mode Packet Trigger status according to the handler instance registers.
  * @param husart Pointer to a \ref hal_usart_handle_t structure which contains the USART instance.
  * @retval Current Autonomous Mode Packet Trigger status.
  */
hal_usart_am_packet_trig_status_t HAL_USART_AM_IsEnabledPacketTrigger(const hal_usart_handle_t *husart)
{
  USART_TypeDef *p_usartx;
  ...
  return (hal_usart_am_packet_trig_status_t)LL_USART_IsEnabled_SelectedTrigger(p_usartx);
}

/**
  * @brief Set the USART Autonomous Packet Trigger configuration set as parameter into the handler instance
  * registers.
  * @param husart Pointer to a \ref hal_usart_handle_t structure which contains the USART instance.
  * @param p_config Pointer to a hal_usart_am_config_t.
  * @retval HAL_OK USART instance has been correctly configured.
  * @retval HAL_INVALID_PARAM p_config is NULL.
  */
hal_status_t HAL_USART_AM_SetConfigPacketTrigger(const hal_usart_handle_t *husart,
                                                 const hal_usart_am_config_t *p_config)
{
  USART_TypeDef *p_usartx;
  ...
  ASSERT_DBG_PARAM(IS_USART_TRIGGER_POLARITY(p_config->trigger_polarity));
  ASSERT_DBG_PARAM(IS_USART_TRIGGER(p_config->source));
  ...
  LL_USART_AM_ConfigPacketTrigger(p_usartx, (uint32_t)p_config->trigger_polarity, (uint32_t)p_config->source,
                                  (uint32_t)p_config->packet_size_byte);
...
/**
  * @brief Get the USART Autonomous Packet Trigger configuration according to the handler instance registers.
  * @param husart Pointer to a \ref hal_usart_handle_t structure which contains the USART instance.
  * @param p_config Pointer to a hal_usart_am_config_t.
  */
void HAL_USART_AM_GetConfigPacketTrigger(const hal_usart_handle_t *husart,
                                         hal_usart_am_config_t *p_config)
{
  USART_TypeDef *p_usartx;
  ...
  uint32_t autocr_reg = LL_USART_READ_REG(p_usartx, AUTOCR);
  uint32_t autocr_val = autocr_reg & (uint32_t)USART_AUTOCR_TRIGSEL;
  p_config->source = (hal_usart_am_trig_source_t)(autocr_val);

  autocr_val = autocr_reg & (uint32_t)USART_AUTOCR_TRIGPOL;
  p_config->trigger_polarity = (hal_usart_am_trig_polarity_t)(autocr_val);

  p_config->packet_size_byte = autocr_reg & USART_AUTOCR_TDN;
}

/**
  * @brief Enable the Autonomous Mode Idle Frame Sending into the handler instance registers.
  * @param husart Pointer to a \ref hal_usart_handle_t structure which contains the USART instance.
  * @retval HAL_OK USART instance has been correctly configured.
  */
hal_status_t HAL_USART_AM_EnableIdle(const hal_usart_handle_t *husart)
{
  USART_TypeDef *p_usartx;
  ...
  LL_USART_Enable_AutonomousSendIdleFrame(p_usartx);
...
/**
  * @brief Disable the Autonomous Mode Idle Frame Sending into the handler instance registers.
  * @param husart Pointer to a \ref hal_usart_handle_t structure which contains the USART instance.
  * @retval HAL_OK USART instance has been correctly configured.
  */
hal_status_t HAL_USART_AM_DisableIdle(const hal_usart_handle_t *husart)
{
  USART_TypeDef *p_usartx;
  ...
  LL_USART_Disable_AutonomousSendIdleFrame(p_usartx);
...
/**
  * @brief Return the Autonomous Mode Idle Frame Sending status according to the handler instance registers.
  * @param husart Pointer to a \ref hal_usart_handle_t structure which contains the USART instance.
  * @retval hal_usart_am_idle_frame_status_t Current Autonomous Mode Idle Frame Sending status.
  */
hal_usart_am_idle_frame_status_t HAL_USART_AM_IsEnabledIdle(const hal_usart_handle_t *husart)
{
  USART_TypeDef *p_usartx;
  ...
  return (hal_usart_am_idle_frame_status_t)
         LL_USART_IsEnabled_AutonomousSendIdleFrame(p_usartx);
}
