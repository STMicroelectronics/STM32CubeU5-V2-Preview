...
#define EXTI_COMP1                      (LL_EXTI_LINE_17)  /*!< EXTI line connected to comparator output: COMP1 */
#if defined(COMP2)
#define EXTI_COMP2                      (LL_EXTI_LINE_18)  /*!< EXTI line connected to comparator output: COMP2 */
#endif /* COMP2 */
...
#define IS_COMP_INPUT_PLUS(input_plus) (((input_plus) == HAL_COMP_INPUT_PLUS_IO1) \
                                        ...
                                        || ((input_plus) == HAL_COMP_INPUT_PLUS_IO4) \
                                        || ((input_plus) == HAL_COMP_INPUT_PLUS_IO5) \
                                        || ((input_plus) == HAL_COMP_INPUT_PLUS_IO6))
#else
#define IS_COMP_INPUT_PLUS(input_plus) (((input_plus) == HAL_COMP_INPUT_PLUS_IO1) \
                                        || ((input_plus) == HAL_COMP_INPUT_PLUS_IO2) \
                                        || ((input_plus) == HAL_COMP_INPUT_PLUS_IO3))
...
/*! Assert definitions of comparator output blanking (specific COMP1) */
#define IS_COMP_OUTPUT_BLANK_COMP1(output_blank) (((output_blank) == HAL_COMP_OUTPUT_BLANK_NONE) \
                                                  ...
                                                  || ((output_blank) == HAL_COMP_OUTPUT_BLANK_TIM3_OC3))

/*! Assert definitions of comparator output blanking (specific COMP2) */
#define IS_COMP_OUTPUT_BLANK_COMP2(output_blank) (((output_blank) == HAL_COMP_OUTPUT_BLANK_NONE) \
                                                  || ((output_blank) == HAL_COMP_OUTPUT_BLANK_TIM3_OC4) \
                                                  || ((output_blank) == HAL_COMP_OUTPUT_BLANK_TIM8_OC5) \
                                                  || ((output_blank) == HAL_COMP_OUTPUT_BLANK_TIM15_OC1))
...
#if defined(USE_HAL_COMP_EXTI) && (USE_HAL_COMP_EXTI == 1)
  LL_EXTI_DisableEvent_0_31(hcomp->exti_line);
  LL_EXTI_DisableIT_0_31(hcomp->exti_line);
  LL_EXTI_DisableRisingTrig_0_31(hcomp->exti_line);
  LL_EXTI_DisableFallingTrig_0_31(hcomp->exti_line);
#endif /* USE_HAL_COMP_EXTI */
...
hal_status_t HAL_COMP_WINDOW_SetHandle(hal_comp_handle_t *hcomp_upper, hal_comp_handle_t *hcomp_lower)
{
  ...
  /* Set handles assignation in window */
...
hal_status_t HAL_COMP_SetConfig(hal_comp_handle_t *hcomp, const hal_comp_config_t *p_config)
{
  ...
  if (LL_COMP_IsLocked(p_instance) == 0UL)
  {
    ...
    reg_config = LL_COMP_READ_REG(p_instance, CSR);
    reg_config &= ~(COMP_CSR_PWRMODE
                    | COMP_CSR_INPSEL
                    | COMP_CSR_INMSEL
                    | COMP_CSR_HYST
                    | COMP_CSR_POLARITY);
    ...
    LL_COMP_WRITE_REG(p_instance, CSR, reg_config);
    ...
    {
      LL_EXTI_EnableRisingTrig_0_31(hcomp->exti_line);
    }
    else
    {
      LL_EXTI_DisableRisingTrig_0_31(hcomp->exti_line);
    ...
    {
      LL_EXTI_EnableFallingTrig_0_31(hcomp->exti_line);
    }
    else
    {
      LL_EXTI_DisableFallingTrig_0_31(hcomp->exti_line);
...
void HAL_COMP_GetConfig(const hal_comp_handle_t *hcomp, hal_comp_config_t *p_config)
{
  ...
  reg_config = LL_COMP_READ_REG(p_instance, CSR);
  p_config->power_mode = (hal_comp_power_mode_t)((uint32_t)(reg_config & COMP_CSR_PWRMODE));
  p_config->input_plus = (hal_comp_input_plus_t)((uint32_t)(reg_config & COMP_CSR_INPSEL));
  p_config->input_minus = (hal_comp_input_minus_t)((uint32_t)(reg_config & COMP_CSR_INMSEL));
  p_config->input_hysteresis = (hal_comp_input_hysteresis_t)((uint32_t)(reg_config & COMP_CSR_HYST));
  p_config->output_polarity = (hal_comp_output_polarity_t)((uint32_t)(reg_config & COMP_CSR_POLARITY));
...
hal_status_t HAL_COMP_WINDOW_SetConfig(hal_comp_handle_t *hcomp, const hal_comp_window_config_t *p_config)
{
  ...
  ASSERT_DBG_PARAM(IS_COMP_POWER_MODE(p_config->power_mode));
  ASSERT_DBG_PARAM(IS_COMP_INPUT_PLUS(p_config->input));
  ASSERT_DBG_PARAM(IS_COMP_INPUT_MINUS(p_config->upper_threshold));
  ASSERT_DBG_PARAM(IS_COMP_INPUT_MINUS(p_config->lower_threshold));
  ASSERT_DBG_PARAM(IS_COMP_INPUT_HYSTERESIS(p_config->input_hysteresis));
  ASSERT_DBG_PARAM(IS_COMP_OUTPUT_POLARITY(p_config->output_polarity));
#if defined(USE_HAL_COMP_EXTI) && (USE_HAL_COMP_EXTI == 1)
  ASSERT_DBG_PARAM(IS_COMP_OUTPUT_TRIG(p_config->output_trigger));
  ...
  else
  {
    /* Set COMP configuration in a single register write access
       (equivalent to successive calls of configuration functions LL_COMP_Set...()) */
    reg_config = LL_COMP_READ_REG(p_instance_upper, CSR);
    reg_config &= ~(COMP_CSR_PWRMODE
                    | COMP_CSR_INPSEL
                    | COMP_CSR_INMSEL
                    | COMP_CSR_HYST
    ...
    reg_config |= ((uint32_t)p_config->power_mode
                   ...
                   | (uint32_t)p_config->input_hysteresis
                   | (uint32_t)p_config->output_polarity);
    LL_COMP_WRITE_REG(p_instance_upper, CSR, reg_config);

    reg_config &= ~(COMP_CSR_INPSEL
                    | COMP_CSR_INMSEL);
    reg_config |= (uint32_t)p_config->lower_threshold;
    LL_COMP_WRITE_REG(p_instance_lower, CSR, reg_config);
    ...
    if (((uint32_t)p_config->output_trigger & (uint32_t)HAL_COMP_OUTPUT_TRIG_RISING) != 0U)
    {
      LL_EXTI_EnableRisingTrig_0_31(hcomp->exti_line | hcomp->p_link_next_handle->exti_line);
    }
    else
    {
      LL_EXTI_DisableRisingTrig_0_31(hcomp->exti_line | hcomp->p_link_next_handle->exti_line);
    }

    if (((uint32_t)p_config->output_trigger & (uint32_t)HAL_COMP_OUTPUT_TRIG_FALLING) != 0U)
    {
      LL_EXTI_EnableFallingTrig_0_31(hcomp->exti_line | hcomp->p_link_next_handle->exti_line);
    }
    else
    {
      LL_EXTI_DisableFallingTrig_0_31(hcomp->exti_line | hcomp->p_link_next_handle->exti_line);
    }
#endif /* USE_HAL_COMP_EXTI */
...
void HAL_COMP_WINDOW_GetConfig(const hal_comp_handle_t *hcomp, hal_comp_window_config_t *p_config)
{
  ...
  reg_config = LL_COMP_READ_REG(p_instance_upper, CSR);
  p_config->power_mode = (hal_comp_power_mode_t)((uint32_t)(reg_config & COMP_CSR_PWRMODE));
  p_config->input = (hal_comp_input_plus_t)((uint32_t)(reg_config & COMP_CSR_INPSEL));
  p_config->input_hysteresis = (hal_comp_input_hysteresis_t)((uint32_t)(reg_config & COMP_CSR_HYST));
  p_config->output_polarity = (hal_comp_output_polarity_t)((uint32_t)(reg_config & COMP_CSR_POLARITY));

  /* Configuration specific to each comparator */
  p_config->upper_threshold = (hal_comp_input_minus_t)((uint32_t)(reg_config & COMP_CSR_INMSEL));
...
hal_status_t HAL_COMP_WINDOW_SetInput(hal_comp_handle_t *hcomp, hal_comp_input_plus_t input)
{
  ...
  ASSERT_DBG_PARAM(hcomp != NULL);
  ASSERT_DBG_PARAM(IS_COMP_INPUT_PLUS(input));
...
hal_status_t HAL_COMP_WINDOW_SetUpperThreshold(hal_comp_handle_t *hcomp, hal_comp_input_minus_t upper_threshold)
{
  ...
  ASSERT_DBG_PARAM(hcomp != NULL);
  ASSERT_DBG_PARAM(IS_COMP_INPUT_MINUS(upper_threshold));
...
hal_status_t HAL_COMP_WINDOW_SetLowerThreshold(hal_comp_handle_t *hcomp, hal_comp_input_minus_t lower_threshold)
{
  ...
  ASSERT_DBG_PARAM(hcomp != NULL);
  ASSERT_DBG_PARAM(IS_COMP_INPUT_MINUS(lower_threshold));
...
void HAL_COMP_IRQHandler(hal_comp_handle_t *hcomp)
{
  ...
  if (LL_EXTI_IsActiveRisingFlag_0_31(hcomp->exti_line) != 0UL)
  {
#if defined(COMP_WINDOW_MODE_SUPPORT)
#if defined(USE_HAL_COMP_WINDOW_MODE) && (USE_HAL_COMP_WINDOW_MODE == 1)
    ...
    if (hcomp->global_state == HAL_COMP_STATE_WINDOW_ACTIVE)
    {
      ...
      LL_EXTI_ClearRisingFlag_0_31(hcomp->exti_line | hcomp->p_link_next_handle->exti_line);
    }
    else
#endif /* USE_HAL_COMP_WINDOW_MODE */
#endif /* COMP_WINDOW_MODE_SUPPORT */
    {
      ...
      LL_EXTI_ClearRisingFlag_0_31(hcomp->exti_line);
    }
  ...
  if (LL_EXTI_IsActiveFallingFlag_0_31(hcomp->exti_line) != 0UL)
  {
#if defined(COMP_WINDOW_MODE_SUPPORT)
#if defined(USE_HAL_COMP_WINDOW_MODE) && (USE_HAL_COMP_WINDOW_MODE == 1)
    /* Check whether comparator is in independent or window mode */
    if (hcomp->global_state == HAL_COMP_STATE_WINDOW_ACTIVE)
    {
      ...
      /* Note: Pair of comparators in window mode can both trig IRQ when input voltage is changing from "out of window"
               area (low or high ) to the other "out of window" area (high or low).
               Both flags must be cleared to call comparator trigger callback is called once. */
      LL_EXTI_ClearFallingFlag_0_31(hcomp->exti_line | hcomp->p_link_next_handle->exti_line);
    }
    else
#endif /* USE_HAL_COMP_WINDOW_MODE */
#endif /* COMP_WINDOW_MODE_SUPPORT */
    {
      /* Clear COMP EXTI line pending bit */
      LL_EXTI_ClearFallingFlag_0_31(hcomp->exti_line);
    }
...
hal_status_t HAL_COMP_Start(hal_comp_handle_t *hcomp)
{
  ...
  else
  {
    if (hcomp->output_trigger != HAL_COMP_OUTPUT_TRIG_NONE)
    {
      LL_EXTI_ClearRisingFlag_0_31(hcomp->exti_line);
      LL_EXTI_ClearFallingFlag_0_31(hcomp->exti_line);
      LL_EXTI_EnableEvent_0_31(hcomp->exti_line);
    }
  }
#endif /* USE_HAL_COMP_EXTI */
...
hal_status_t HAL_COMP_Start_IT(hal_comp_handle_t *hcomp)
{
  ...
  if (status == HAL_OK)
  {
    LL_EXTI_ClearRisingFlag_0_31(hcomp->exti_line);
    LL_EXTI_ClearFallingFlag_0_31(hcomp->exti_line);
    LL_EXTI_EnableIT_0_31(hcomp->exti_line);
  }
  else
  {
    hcomp->global_state = HAL_COMP_STATE_IDLE;
...
hal_status_t HAL_COMP_WINDOW_Start(hal_comp_handle_t *hcomp)
{
  ...
  else
  {
    if (hcomp->output_trigger != HAL_COMP_OUTPUT_TRIG_NONE)
    {
      LL_EXTI_ClearRisingFlag_0_31(hcomp->exti_line | hcomp->p_link_next_handle->exti_line);
      LL_EXTI_ClearFallingFlag_0_31(hcomp->exti_line | hcomp->p_link_next_handle->exti_line);
      LL_EXTI_EnableEvent_0_31(hcomp->exti_line | hcomp->p_link_next_handle->exti_line);
    }
  }
#endif /* USE_HAL_COMP_EXTI */
...
hal_status_t HAL_COMP_WINDOW_Stop(hal_comp_handle_t *hcomp)
{
  ...
  if (LL_COMP_IsLocked(COMP_GET_INSTANCE(hcomp)) == 0UL)
  {
    ...
    if (hcomp->output_trigger != HAL_COMP_OUTPUT_TRIG_NONE)
    {
      LL_EXTI_DisableEvent_0_31(hcomp->exti_line | hcomp->p_link_next_handle->exti_line);
    }
#endif /* USE_HAL_COMP_EXTI */
...
#if defined(USE_HAL_COMP_EXTI) && (USE_HAL_COMP_EXTI == 1)
/**
  * @brief  Start window comparators with interruption: default interruptions.
  * @param  hcomp Pointer to a hal_comp_handle_t structure.
...
hal_status_t HAL_COMP_WINDOW_Start_IT(hal_comp_handle_t *hcomp)
{
  ...
  if (status == HAL_OK)
  {
    LL_EXTI_ClearRisingFlag_0_31(hcomp->exti_line | hcomp->p_link_next_handle->exti_line);
    LL_EXTI_ClearFallingFlag_0_31(hcomp->exti_line | hcomp->p_link_next_handle->exti_line);
    LL_EXTI_EnableIT_0_31(hcomp->exti_line | hcomp->p_link_next_handle->exti_line);
  }
  else
  {
    hcomp->global_state = HAL_COMP_STATE_WINDOW_IDLE;
...
/**
  * @brief  Stop window comparators in interruption mode.
  * @param  hcomp Pointer to a HAL_COMP_WINDOW_handle_t structure.
  * @retval HAL_ERROR         Operation completed with error.
  * @retval HAL_OK            Operation completed successfully.
  */
hal_status_t HAL_COMP_WINDOW_Stop_IT(hal_comp_handle_t *hcomp)
{
  ...
  if (LL_COMP_IsLocked(COMP_GET_INSTANCE(hcomp)) == 0UL)
  {
    ...
    LL_EXTI_DisableIT_0_31(hcomp->exti_line | hcomp->p_link_next_handle->exti_line);
